...
class GuestCheckout(BasePage):
    ...
    def fill_guest_info(self, first_name, last_name, email, telephone, street, city, zip_code):
        WebDriverWait(self.driver, 20).until(
            EC.visibility_of_element_located(self.FIRST_NAME_TF)
        )
        self.fill_text(self.FIRST_NAME_TF, first_name)
        self.fill_text(self.LAST_NAME_TF, last_name)
        self.fill_text(self.EMAIL_TF, email)
        self.fill_text(self.PHONE_NUMBER_TF,telephone)
        self.fill_text(self.ADDRESS_TF, street)
        self.fill_text(self.CITY_TF, city)
        self.fill_text(self.ZIP_CODE_TF, zip_code)
